...
# 生成新的序列
def generate_sequence(root_node,level, childrens_list,children_dict):
    global sequence_id
    ...
    if len(children_node) < children_dict[level+1]:
        ...
        children_node = root_node.getchildren()
    print(sequence_id,root_node, len(children_node), level, children_dict[level + 1])
    for child in children_node:
        generate_sequence(child, level + 1, childrens_list,children_dict)
    return
...
def getXMlSequence(file_name,children_dict):
    ...
    generate_sequence(root, level, childrens_list, children_dict)
